...
public class WhatsappPayloadBuilder {
    ...
    /**
     * Converts raw variables list → [{type: text, text: value}, ...]
     * Handles both object form {value: "..."} and plain string form.
     */
    ...
    private void buildCarouselComponents(List<Map<String, Object>> components,
        ...
        for (int i = 0; i < carouselCards.size(); i++) {
            ...
            cards.add(Map.of("card_index", i, "components", cardComponents));
        }

        components.add(Map.of("type", "body", "parameters", bodyParameters));
        components.add(Map.of("type", "carousel", "cards", cards));
    }
    ...
    private void buildCarouselImageHeader(List<Map<String, Object>> cardComponents,
        ...
        cardComponents.add(Map.of(
                ...
                "parameters", List.of(Map.of(
                        "type", "image",
                        "image", Map.of("id", mediaId != null ? mediaId : "")
                ))
        ));
    }
    ...
    private void buildCarouselBody(List<Map<String, Object>> cardComponents, CarouselCard card) {
        ...
        // mirrors PHP ksort($vars) — sort by key before building params
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  BUTTON PARAMETERS  (shared for carousel buttons)
    // ════════════════════════════════════════════════════════════════════════
    ...
    /**
     * Safe extractor for Map<?,?>.
     * getOrDefault(String, T) is a compile error on Map<?,?> because key type
     * is unknown wildcard. map.get(Object) works fine — use + null-check instead.
     */
